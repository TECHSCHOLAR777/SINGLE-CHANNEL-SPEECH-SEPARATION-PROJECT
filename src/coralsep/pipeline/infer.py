"""
Full CoRAL-Sep inference pipeline (Dev C, P4-C3).
...
class PipelineResult:
    ...
    def to_separation_result(self, sample_rate: int = 16000) -> SeparationResult:
        ...
        elif sample_rate == CORALSEP_SAMPLE_RATE:
            streams, mixture = self.streams_8k, self.mixture_8k
        else:
            raise ValueError(
                f"sample_rate must be {CORALSEP_SAMPLE_RATE} or 16000, got {sample_rate}"
            )
...
class CoralSepPipeline:
    """
    End-to-end CoRAL-Sep inference pipeline.
    ...
    def run(
        ...
        """
        Run the full CoRAL-Sep pipeline on a mono mixture.
        ...
        proc = coralsep_preprocess(mixture, sample_rate, target_dbfs=self.cfg.target_dbfs)
...
def _level1_condition(proc: CoralSepPreprocessedAudio) -> dict:
