...
def _status_html(base_time: float, calm_time: float, temperature: float, n_det: int) -> str:
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;padding:12px 0 4px;">
      ...
      <span style="font-size:0.72rem;padding:4px 12px;border-radius:100px;
        ...
        CoRAL-Sep &nbsp;<strong>{calm_time:.1f}s</strong>
      </span>
      <span style="font-size:0.72rem;padding:4px 12px;border-radius:100px;
...
def process(audio_input, progress=gr.Progress(track_tqdm=False)):
    ...
    progress(0.55, desc="Running CoRAL-Sep …")
...
<div class="cs-hero" id="hero">
  ...
  <!-- Three waveform lines: mix (white) → baseline (indigo) → CoRAL-Sep (teal) -->
    ...
    <div class="cs-bench-grid">
      <div class="cs-bench-card">
        ...
        <table class="cs-btable">
          <thead>
            <tr><th>Metric</th><th>SR-CorrNet</th><th>CoRAL-Sep</th><th>Δ</th></tr>
          </thead>
          <tbody>
...
"""
    with gr.Blocks(css=CSS, title="CoRAL-Sep", theme=_make_theme(), js=_sync_js) as demo:
        ...
        with gr.Group(elem_id="cs-demo-content"):
            ...
            with gr.Row(equal_height=False):
                ...
                with gr.Column():
                    gr.HTML(
                        '<div class="cs-col-head"><div class="cs-col-dot" style="background:#00D4B8"></div><span class="cs-col-title" style="color:#00D4B8">CoRAL-Sep · Gate + LoRA</span></div>'
                    )
