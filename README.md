...
> **Last updated:** 2026-07-13 (**first real training run complete on Kaggle T4** — 30 epochs, loss 0.60 → 0.02, checkpoint saved. **P2-INT3 done** (converged), **P2-INT5 done** (escalation 57% measured). **P2-INT4 measured but does NOT yet pass**: cascade 10.43 dB vs SR-CorrNet-alone 14.84 dB SI-SDRi — the cascade beats the cheap expert (+5.2) but loses to the expensive one. **A `gate_tau` sweep was then run and RULED OUT as the fix** — even at 100% escalation the fused output (14.21 dB) plateaus just below raw SR-CorrNet (14.84): on clean all-3-speaker audio the fusion slightly degrades an already-strong expert, and the cheap 2-speaker model can't contribute. Beating the best single expert needs the mixed-N / noisy regime the cascade was designed for, not tuning (see P2-INT4). Reaching a running train step took fixing **7** distinct live bugs CI structurally could not see (SR-CorrNet HF-load, ECAPA reload, REAL-M 2-source, `max_tracks` cap, 2× CUDA device placement in the trainer, and the ECAPA-192-vs-64 loss dim). **M1 fully passed**; **M2 in progress** (2 of the training criteria green; "beats single expert" is a data-regime task, not a tune). P1 GPU confirmation (cell 6) hit a CUDA OOM — non-blocking, P1-INT2 already closed via CI. Pulse 93/19/131)
...
| Fusion loses to SR-CorrNet alone | Medium | Fall back to ensemble / SR-CorrNet-primary | MASTER §5.3 at M2 | ![in progress](https://img.shields.io/badge/in_progress-yellow?style=flat-square) [~] — **materialized + confirmed by tau sweep 2026-07-13**: even at 100% escalation the fused output is 14.21 dB vs raw SR-CorrNet 14.84 (P2-INT4). On clean audio the CRRR fusion slightly degrades an already-strong expert. This is real but scoped to the clean regime — the residual correction has nothing to fix when SR-CorrNet is near-perfect. Watch on noisy/reverberant + mixed-N data (P4); the ensemble/SR-primary fallback is the mitigation if it persists there |
