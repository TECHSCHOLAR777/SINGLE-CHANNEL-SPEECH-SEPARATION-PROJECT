...
> **Last updated:** 2026-07-13 (**first real training run complete on Kaggle T4** — 30 epochs, loss 0.60 → 0.02, checkpoint saved. **P2-INT3 done** (converged), **P2-INT5 done** (escalation 57% measured). **P2-INT4 measured but does NOT yet pass**: cascade 10.43 dB vs SR-CorrNet-alone 14.84 dB SI-SDRi — the cascade beats the cheap expert (+5.2) but under-escalates (43% stay on the weak expert at tau=12), so it loses to the expensive expert; fix is a `gate_tau` sweep on the existing checkpoint, not a rebuild. Reaching a running train step took fixing **7** distinct live bugs CI structurally could not see (SR-CorrNet HF-load, ECAPA reload, REAL-M 2-source, `max_tracks` cap, 2× CUDA device placement in the trainer, and the ECAPA-192-vs-64 loss dim). **M1 fully passed**; **M2 in progress** (2 of the training criteria green, "beats single expert" pending the tau sweep). P1 GPU confirmation (cell 6) hit a CUDA OOM — non-blocking, P1-INT2 already closed via CI. Pulse 93/19/131)
...
| P2-INT4 | Validate: beats best single expert on val set | P2-INT3 | B + C | Metric comparison table | ![not done yet](https://img.shields.io/badge/not_done_yet-red?style=flat-square) [ ] — **measured 2026-07-13, does NOT yet pass.** PIT SI-SDRi on 100 dev samples: **cascade 10.43 dB**, MossFormer2 (cheap) 5.26 dB, SR-CorrNet (expensive) **14.84 dB**. `cascade_beats_single_expert: false` — the cascade beats the cheap expert by +5.17 dB but loses to the expensive expert alone by −4.41 dB. Root cause is diagnostic, not broken: at `gate_tau=12`, only **57% of samples escalate**, so ~43% are answered by the 5 dB cheap expert (`0.57×14.8 + 0.43×5.3 ≈ 10.7` ≈ the observed 10.43) — the fusion on escalated samples roughly matches SR-CorrNet, the loss is entirely the under-escalation. Next: sweep `gate_tau` down to escalate more, and/or train the gate; this is a tuning task on a real checkpoint, not a rebuild |
...
- ![not done yet](https://img.shields.io/badge/not_done_yet-red?style=flat-square) [ ] **Beats best single expert on mixed-condition validation** — measured, does NOT pass yet: cascade 10.43 dB vs expensive 14.84 dB SI-SDRi (−4.41). Under-escalation at tau=12 is the cause; tau sweep pending (see P2-INT4)
...
| Fusion loses to SR-CorrNet alone | Medium | Fall back to ensemble / SR-CorrNet-primary | MASTER §5.3 at M2 | ![in progress](https://img.shields.io/badge/in_progress-yellow?style=flat-square) [~] — **materialized on the first trained run 2026-07-13**: cascade 10.43 dB < SR-CorrNet 14.84 dB (P2-INT4). But the diagnosis is under-escalation (43% of samples stay on the 5 dB cheap expert at tau=12), not the fusion being worse than SR-CorrNet — on escalated samples the fused output roughly matches SR-CorrNet. Mitigation is the tau sweep, not yet the ensemble fallback |
