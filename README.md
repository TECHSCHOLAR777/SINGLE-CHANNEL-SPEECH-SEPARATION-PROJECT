...
> **Last updated:** 2026-07-13 (**first real training run complete on Kaggle T4** — 30 epochs, loss 0.60 → 0.02, checkpoint saved. **P2-INT3 done** (converged), **P2-INT5 done** (escalation 57% measured). **P2-INT4 measured but does NOT yet pass**: cascade 10.43 dB vs SR-CorrNet-alone 14.84 dB SI-SDRi — the cascade beats the cheap expert (+5.2) but loses to the expensive one. **A `gate_tau` sweep was then run and RULED OUT as the fix** — even at 100% escalation the fused output (14.21 dB) plateaus just below raw SR-CorrNet (14.84): on clean all-3-speaker audio the fusion slightly degrades an already-strong expert, and the cheap 2-speaker model can't contribute. Beating the best single expert needs the mixed-N / noisy regime the cascade was designed for, not tuning (see P2-INT4). Reaching a running train step took fixing **7** distinct live bugs CI structurally could not see (SR-CorrNet HF-load, ECAPA reload, REAL-M 2-source, `max_tracks` cap, 2× CUDA device placement in the trainer, and the ECAPA-192-vs-64 loss dim). **M1 fully passed**; **M2 in progress** — but "beats single expert" is now a confirmed NEGATIVE on both clean and mixed 2–5 spk (cascade ~13.1 < SR-CorrNet ~16.2, fusion degrades the expert even at 100% escalation). This is the real M2 blocker and needs a fusion fix / SR-primary bypass / noisy data, not tuning. **M3 counting pipeline built + a 10th CUDA bug (fit_temperature device) fixed; counting run pending a re-run to emit the confusion-matrix + calibration artifacts.** P1 GPU confirmation (cell 6) hit a CUDA OOM — non-blocking, P1-INT2 already closed via CI. **Since then (same day): mixed 2–5 spk pipeline built and run** — the mixed-N result confirmed the negative (cascade ~13.1 < SR-CorrNet ~16.2, fusion degrades even at 100% escalation); fixing it uncovered bugs #8 (variable-N reference collate), #9 (O(K!) PIT loss stalling the GPU — replaced with a Hungarian assignment, numerically identical, ~1000× faster), and #10 (fit_temperature device). **`scripts/evaluate_cascade --sr-primary`** now provides the honest cascade (escalated→raw SR-CorrNet, else MossFormer2, no fusion, no retrain) — the correct novelty ablation. **Full P3 counting pipeline built** (`scripts/train_stop_classifier.py`, `scripts/eval_counting.py`, `eval/counting_infer.py`) — trains the stop-classifier off the cache and emits the M3 confusion-matrix + calibration artifacts; one GPU run from closing M3's data criteria. **`notebooks/kaggle_p2.py` restructured** into a clean top-to-bottom Save-&-Run-All pipeline for the team. Pulse 93/19/131)
...
| P2-INT3 | Short training run (few epochs) on mixed conditions | P2-INT2 | B | Checkpoint + logs | ![not done yet](https://img.shields.io/badge/not_done_yet-red?style=flat-square) [ ] — **was structurally impossible until 2026-07-12**: `CRRRFusionHead.forward` raises `stream shape mismatch` the instant a 2-stream MossFormer2 output meets a 3-stream SR-CorrNet output on a 3-speaker mixture, which every Libri3Mix training batch would trigger. Fixed via `MossFormer2Expert(target_speakers=k)`: the missing stream is filled with the RESIDUAL (mixture minus the sum of emitted streams), which on a genuine speaker gap recovers something close to the missed speaker rather than being pure padding — tested to 1e-5 in `tests/test_mossformer2_residual.py` (6 tests). Marked `synthetic="residual"`, confidence 0.0, so downstream heads learn to distrust it; residual energy is also a free escalation signal. Blocker is dead. **DONE 2026-07-13:** first real training run executed on Kaggle T4 — 30 epochs on the 500-sample frozen-expert cache, loss converged 0.5995 → 0.0208, checkpoint saved (`outputs/training/checkpoint.pt` + `checkpoint.json`). Getting here required fixing 7 live bugs the real run exposed (see the Last-updated note). Note: train loss this low on a 500-sample clean smoke likely reflects some overfitting — expected at this scale; the val numbers are in P2-INT4 |
| P2-INT4 | Validate: beats best single expert on val set | P2-INT3 | B + C | Metric comparison table | ![not done yet](https://img.shields.io/badge/not_done_yet-red?style=flat-square) [ ] — **measured 2026-07-13, does NOT yet pass.** PIT SI-SDRi on 100 dev samples: **cascade 10.43 dB**, MossFormer2 (cheap) 5.26 dB, SR-CorrNet (expensive) **14.84 dB**. `cascade_beats_single_expert: false`. **`gate_tau` sweep run 2026-07-13 (via `evaluate_cascade --tau`) — a tuning fix is ruled out:** as escalation rises 51%→100% the cascade climbs 9.81→**14.21 dB** but **plateaus just below SR-CorrNet's 14.84 even at 100% escalation** (tau: 12→10.43, 20→10.69, 100→14.21; expensive 14.84 throughout). Two honest conclusions: (1) at 100% escalation the fused output (14.21) is ~0.6 dB **worse** than raw SR-CorrNet — the CRRR fusion, trained on a 500-sample clean smoke, slightly degrades an already-strong expert rather than improving it; (2) below 100% escalation the cheap expert — MossFormer2, a **2-speaker** model — is so weak on these **all-3-speaker** mixtures (5.26 dB) that every non-escalated sample tanks the average. So beating the best single expert is **not reachable on clean all-3-speaker data by tuning this checkpoint**. The cascade's thesis needs the regime it was designed for: a **mixed 2–5 speaker** eval (cheap expert competent on 2-spk cases) and/or noisy/reverberant conditions where the residual fusion genuinely helps. That is a data-regime + fusion-training task, not a tau tweak |
| P2-INT5 | Measure and report escalation rate | P2-B2, P2-INT3 | C | Escalation rate per tier | ![done](https://img.shields.io/badge/done-brightgreen?style=flat-square) [x] — measured 2026-07-13 on the trained checkpoint: **escalation rate 57%** on the 100-sample dev set (`gate_tau=12`, REAL-M min-SI-SNR signal), reported by `scripts/evaluate_cascade.py`. This is above the 30–40% target — the tau sweep that fixes P2-INT4 will also bring this toward target |
...
- ![not done yet](https://img.shields.io/badge/not_done_yet-red?style=flat-square) [ ] **Beats best single expert on mixed-condition validation** — does NOT pass, now confirmed on BOTH regimes. Clean 3-spk: cascade 14.21 < SR-CorrNet 14.84. **Mixed 2–5 spk (2026-07-13, the regime this criterion specifies): cascade best ~13.1 dB (tau=10) vs SR-CorrNet ~16.2 dB — `beats=False` at every tau, incl. 100% escalation (~12.5 < 16.2).** The fusion degrades the expensive expert by ~3–4 dB even when always escalating, so this is not a routing/tau problem — the CRRR fusion head, trained on a 500-sample smoke, actively hurts an already-strong SR-CorrNet. Beating the best single expert needs real work (bigger training set, a fusion redesign, or a "bypass fusion / SR-primary when escalated" path), or the noisy/reverberant regime where the residual correction has something to fix. See P2-INT4 |
