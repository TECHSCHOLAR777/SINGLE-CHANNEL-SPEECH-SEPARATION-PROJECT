...
**Status:** [AMBER] Most artifacts are still external and none is hash-verified, but the backbone's provenance is resolved (I-019, closed) and a real GPU with real Kaggle access reached several artifacts directly this session. See section 5 for what changed.
...
| DATA-004 | LibriMix, `wav8k/min/test`, `mix_both` | evaluation | LibriMix generator over LibriSpeech and WHAM! | 30 clips per split, 2, 3 and 5 speakers | supplied by argument | no | [PARTIALLY_VERIFIED] via result JSON. Searched Kaggle broadly for a public substitute this session: found only `mix_clean`-only, single-N mirrors (`trngttrng12/librimix-eval`, Libri2Mix dev/test only; `garvs777/libri3mix`, appears to be the same `mix_clean`-only shape). Neither has `mix_both` or N ∈ {3, 4, 5} together, so neither can stand in for this dataset. Generating the real thing needs the official LibriMix generator over LibriSpeech `test-clean` and WHAM's `tt` split, not yet done |
...
| Kaggle, public, unverified authors | Two `mix_clean`-only LibriMix mirrors, two full WHAM! mirrors | reachable; the WHAM mirror `ngcthun/wham-noise` (19.5 GB) has the correct split layout for I-044 and has not yet been downloaded; the LibriMix mirrors do not cover this project's needs (see DATA-004) |
...
| Original author's local machine or the full public corpora | The complete DATA-001/002/003 (137,876 utterances, 28,000 noise clips, 10,001 RIRs) and the official LibriMix test set (DATA-004) | not reached this session; only a small slice of DATA-001 and a 320-response diagnostic RIR bank exist here, generated fresh rather than recovered |
