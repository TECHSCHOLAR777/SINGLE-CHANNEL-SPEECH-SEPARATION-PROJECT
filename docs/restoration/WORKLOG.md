...
**Phase:** 8 continued. Noise and codec adapters independently evaluated for the first time (I-057), a real audio-corruption bug found and fixed in the codec pipeline (I-058), a real feasibility check on retraining SR-CorrNet itself (I-059), and CI made green on `master` for the first time in this project's history (I-056).

**Actions and findings, in the order they happened.**

Built `eval_degradation_adapter.py`, generalizing the reverb diagnostic for adapters whose damage does not touch the reference. Ran it against the real noise and codec checkpoints: both are neutral to helpful (noise +2.05 dB mild / +0.55 dB severe; codec initially read as +5.20 dB on Opus, +0.25 dB on AMR-NB). The Opus number looked too good and its underlying mixture-vs-reference lower bound was -52.84 dB, physically implausible. Traced it to `codec_augmentation.py`'s decode step: Opus's container always reports its fixed 48 kHz internal clock on decode regardless of the 8 kHz input fed to the encoder, and the code only resampled the decode step back to the source rate when the codec had a mandatory encode-side rate (true for AMR, never true for Opus). `_fit_length` then silently kept only the first sixth of the real decoded audio. Confirmed directly via `sf.info` (48000 Hz, 96000 frames for a 2s/8kHz input) and cross-correlation against broadband noise (near-zero correlation before the fix, 0.95 after, matching AAC's own already-correct roundtrip). Fixed by always resampling the decode step, unconditionally (I-058). Reran the codec diagnostic: the real Opus number is +0.09 dB, essentially neutral, not +5.20 dB.

Since I-058 meant the already-generated 3300-file fixed evaluation matrix's 574 real-Opus samples were silently corrupted, regenerated the full set with the fix, identical seed and inputs. Preserved the old set, did not delete it (`fixed_eval_real_pre_i058_corrupted_opus`). The new set's 554 real Opus samples have realistic durations (2.7s to 35s), confirmed directly by loading real `.npz` files, not by trusting the generator's exit code.

Ran a real feasibility smoke test for retraining SR-CorrNet's own backbone on LibriMix (I-059), the owner's own suggestion for a genuinely fair baseline-vs-CoRAL-Sep comparison. The pip package this project depends on ships inference-only code; the full upstream repo has a real training pipeline. Generated a genuine small Libri2Mix set from real staged data, in an isolated conda environment kept fully separate from the one every other verified result in this project uses (a brief accidental editable-install of the full repo into the shared environment was caught and reverted before it could contaminate anything). Hit and fixed six real environment and config gaps in sequence, the last of which (an off-by-one in epoch counting that silently produces zero real training steps) would have been reported as a false success had the near-zero elapsed time between "start" and "done" log lines not been checked. Ended with a real, loadable 56 MB checkpoint. A full retrain is a separate, multi-day, GPU-cost decision, not yet scoped or launched.

Separately, chased CI to a real green run for the first time in the project's visible history (I-056), which had been red since the workflow was added. Four independent causes, each invisible until the previous one was fixed and a new run actually happened: the missing `demo` extra (`gradio`), `tomllib` not existing on Python 3.10 plus a false positive in the dependency-coverage test's own static import scanner, two tests that download the real SR-CorrNet checkpoint from Hugging Face Hub with no network-failure skip (failed on a transient 429 rate limit), and a `from conftest import` that does not reliably resolve under this project's pytest path configuration, found only because fixing the previous cause surfaced it in the next CI run. Converted the last fix to a proper pytest fixture, the mechanism this project's other shared test helpers already use, and verified it actually injects via a throwaway consumer test before removing it, rather than trusting the pattern by precedent alone.

While preparing to rerun I-043's co-activation check against the fixed reverb adapter, found the same unseeded-`RirBank` defect as I-055 independently present in `diagnose_coactivation.py`. Fixed it, then reran using the samples2000 ablation checkpoint (rank 8, compatible with this script's single shared-rank `LoRALibrary`) instead of the harmful original. Result: a real -3.85 dB co-activation cost, the opposite of the 2026-09-04 "ruled out" verdict (-0.03 dB), which was measured on the harmful checkpoint and likely hit a floor effect (little headroom left to lose when the adapter is already bad in both regimes). Reopened I-043.

**Findings.** The CI-greening exercise is its own case study in the discipline this project keeps re-learning: fixing one visible cause does not mean the causes are exhausted, and each fix must be verified against a real subsequent run before the ticket can close, not predicted. The I-043 reversal is a sharper version of the same lesson at the modeling level: a null result measured on a broken component does not generalize to the fixed component, and closing a ticket on a negative finding needs the same scrutiny as closing one on a positive finding.

**Issues opened and closed this entry.** I-058 (opened and closed). I-059 (opened, investigating). I-057 closed. I-056 closed. I-043 reopened.

**Validation.** Full suite locally: 605 passed, 11 skipped throughout. CI run `33958227401` on `master`: Credential scan, Lint, and Test on Python 3.10, 3.11, and 3.12 all passed, confirmed by reading the run's own job list, not assumed from a push succeeding.

**Next action.** I-043's reopened co-activation cost and I-025's rank/sample-count fix are now two independent, both-real levers on the reverb adapter's quality; neither has been checked against the noise/codec adapters' own co-activation cost, and I-043's new number is a single mixture/seed measurement. The rank32 and combined rank32+2000-sample checkpoints still cannot be checked in `diagnose_coactivation.py` without adding per-adapter rank support, since that script's `LoRALibrary` currently applies one shared rank to all three adapters at once.

---

## 2026-09-05, entry 13

**Phase:** 8 continued. Wired the real fixed evaluation matrix into a scoring pipeline (I-060), which surfaced a fundamental, previously-undiscovered bug in `CoralSepPipeline` itself (I-061).

**Actions and findings, in the order they happened.**

`eval/matrix.py`'s only entry point expected a JSONL manifest with separate on-disk wav files; the real generator that actually exists writes a different, self-contained `.npz` format instead, confirmed by loading a real manifest and sample directly rather than trusting the module's own (stale) docstring. Added `run_eval_matrix_npz`, reusing the existing scoring machinery, with 4 new unit tests against synthetic fixtures (the module had zero test coverage before this).

Attempted a real first run against the actual `fixed_eval_real` dataset on the GPU box, per the standing instruction to keep GPU work real and verified. Assembled the only configuration of `CoralSepPipeline` that has real precedent anywhere in this codebase, the bare frozen-backbone `expert` with nothing else attached, matching `demo/cli.py`, the only other place this class is constructed with real (non-mock) components. The run completed and scored 66 real mixtures, but every single one came back with SI-SDRi between -15 and -60 dB, including cases where the reported speaker count was correct, which ruled out a simple counting error as the explanation on its own.

Rather than write these numbers down as a real evaluation result, root-caused them first. Isolated the corruption step by step on one real held-out clip, on the GPU box, comparing against the frozen backbone's own SI-SNR (already known-good from this session's many earlier diagnostics): calling the expert directly, with either an oracle or a dynamic speaker count, scored a sane 0.97 to 18.4 dB. Calling it on the pipeline's own preprocessed audio, and again on a real padded chunk object built by the actual `Chunker` class, both still scored sane, around 18.5 dB, ruling out preprocessing and chunking. The full `CoralSepPipeline.run()`, even restricted to a clip short enough to produce exactly one chunk (ruling out cross-chunk stitching specifically), still scored -35.5 dB. That isolated the corruption to code that executes even without multi-chunk stitching, and reading `ChunkStitcher.__init__` next to `SRCorrNetExpert.separate()` found the exact mechanism directly: the stitcher is constructed with `sample_rate=8000` and computes its internal chunk/step lengths in 8 kHz sample counts, but the per-chunk separation result it is fed is always at 16 kHz by the expert's own documented contract, with nothing in between resampling it back down. The overlap-add reconstruction then takes only the first half, in real time, of each chunk's samples and stitches it as if it were the full chunk, scrambling the entire output.

Filed as I-061, P0. Deliberately did not implement a fix in the same pass: there are at least two structurally different ways to close the gap (resample the per-chunk output down to 8 kHz before the stitcher, matching the `streams_8k`/`streams_16k` naming already used elsewhere in this file; or reconfigure the stitcher to operate natively at 16 kHz and reconsider what the separate band-recovery step is then supposed to add), and picking between them needs a real design decision, not a guess made under time pressure.

**Findings.** `demo/cli.py` is the only place in this entire codebase where `CoralSepPipeline` has ever been assembled with a real (non-mock) expert. Nothing suggests its output was ever quantitatively checked against a reference; a scrambled-timeline bug like this one would not obviously announce itself on short, casually-listened-to demo clips. This means the "assembled pipeline" this project's documentation refers to throughout has, as far as this restoration has found, never actually been verified end to end with a real number, only in isolated per-stage diagnostics, each of which this session confirmed individually works, and each of which (by design or accident) bypasses the exact code path where the bug lives.

**Issues opened and closed this entry.** I-060 (opened, partially blocked). I-061 (opened, root cause found, fix pending).

**Validation.** Full suite locally: 609 passed, 11 skipped. All isolation-step numbers (0.97, 18.4, 18.4, 18.5, -35.5 dB, and the original -15 to -60 dB range) were read directly from real GPU-box output, not estimated or assumed; `ChunkStitcher.__init__` and `SRCorrNetExpert.separate()` were read directly to confirm the exact rate-mismatch arithmetic before concluding anything.

**Next action.** I-061 needs a deliberate fix decision (which of the two candidate designs, or a third option not yet investigated: giving `SRCorrNetExpert` a way to return its native 8 kHz output for pipeline-internal use). Once fixed, I-060's real run should be repeated to get the actual first trustworthy quantitative read on the assembled pipeline, which would then finally be positioned to close I-002/I-023/I-026's long-standing evidence gap.
