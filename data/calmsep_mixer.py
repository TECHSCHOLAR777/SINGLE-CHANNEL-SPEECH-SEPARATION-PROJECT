"""
CALM-Sep 8 kHz dynamic mixer with degradation recipe logging (Dev A, P0-A1).

The frozen SR-CorrNet var-2-5 checkpoint operates at 8 kHz (STFT window 128,
hop 64). Every training and evaluation mixture in CALM-Sep is therefore mixed
...
class CalmSepMixer:
    ...
    sample_rate:
        Expected input rate. Defaults to CALMSEP_SAMPLE_RATE (8000). A file at
    ...
    def __init__(
        ...
        if bad:
            raise ValueError(
                f"allowed_n contains {bad}; CALM-Sep supports N in {{2,3,4,5}} only "
    ...
    def mix(self, split: str = "train", n: int | None = None) -> CalmSepMixture:
        """
        ...
        Returns:
            CalmSepMixture with an anechoic, noise-free, uncompressed mixture
