...
``data/fixed_eval/``. Does not require corpora on disk: each row records the
...
MANIFEST_VERSION: str = "calmsep-fixed-eval-v1"
...
def build_eval_manifest(
    ...
    Args:
        ...
        out_dir: Output directory (typically ``data/fixed_eval/``).
...
def generate_all_manifests(
    ...
    Args:
        out_dir: Root directory for manifests (``data/fixed_eval/``).
