...
# Ensure the project root is importable when run as `python data/fixed_eval_generator.py`
_PROJECT_ROOT = Path(__file__).resolve().parent.parent
if str(_PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(_PROJECT_ROOT))

from data.calmsep_mixer import CALMSEP_SAMPLE_RATE, BAND_RECOVERY_SAMPLE_RATE, CalmSepMixer
from data.degradations import apply_reverb, apply_noise, apply_codec
from data.rir_bank import RirBank, RirRecord, measure_t60, find_direct_path_peak
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate the seeded, hashed, fixed CALM-Sep evaluation matrix.",
    ...
    if manifest_path.exists():
        print(
            ...
            "To verify the existing set: python utils/hashing.py verify "
            f"{manifest_path}"
        )
        raise SystemExit(1)
    ...
    # because that would leave the train pool empty and CalmSepMixer raises.
    ...
    but_bank = _load_but_bank(but_dir, CALMSEP_SAMPLE_RATE)
