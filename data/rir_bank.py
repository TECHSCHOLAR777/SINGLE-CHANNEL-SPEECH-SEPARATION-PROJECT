...
from data.calmsep_mixer import CALMSEP_SAMPLE_RATE
...
class RirRecord:
    ...
    Attributes:
        ...
        sample_rate: Always CALMSEP_SAMPLE_RATE.
    """
    ...
    max_order: int
    n_peak: int
    sample_rate: int = CALMSEP_SAMPLE_RATE

    def to_dict(self) -> dict[str, Any]:
        return asdict(self)


def measure_t60(rir: np.ndarray, sample_rate: int = CALMSEP_SAMPLE_RATE) -> float:
    """
    ...
    def __init__(self, bank_dir: str | Path, rng: np.random.Generator | None = None) -> None:
        ...
        if not index_path.exists():
            raise FileNotFoundError(
                ...
                f"  python -m data.rir_bank --output {self.bank_dir}"
            )
...
def _main() -> None:
    """CLI: python -m data.rir_bank --output data/rirs --per-bucket 1250"""
    import argparse

    parser = argparse.ArgumentParser(description="Generate the CALM-Sep simulated RIR bank.")
    parser.add_argument("--output", default="data/rirs", help="Output directory")
    parser.add_argument(
        "--per-bucket",
        type=int,
    ...
    parser.add_argument(
        "--sample-rate", type=int, default=CALMSEP_SAMPLE_RATE, help="Output sample rate"
    )
