"""
Voiced-frame density for CALM-Sep Level-1 condition analysis (BLUEPRINT §5.4).
...
def _silero_voiced_density(waveform: np.ndarray, sr: int) -> float | None:
    ...
    if sr != CALMSEP_SR:
        ...
        n_out = int(round(duration * CALMSEP_SR))
...
def voiced_frame_density(waveform: np.ndarray, sr: int = CALMSEP_SR) -> float:
    """
    Fraction of the signal flagged as voiced (SileroVAD or STFT fallback).

    Args:
        waveform: Mono waveform, shape [T].
        sr: Sample rate in Hz; default 8000 (CALM-Sep operating rate).
