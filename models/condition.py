...
_BANDWIDTH_BIN = int(round(_BANDWIDTH_CUTOFF_HZ / (CALMSEP_SR / 2) * (STFT_BINS - 1)))
...
def voiced_density_silero(waveform: torch.Tensor, sr: int = CALMSEP_SR) -> float:
    """
...
def voiced_density_energy(waveform: torch.Tensor) -> float:
    ...
    """
    wav = waveform.float().squeeze()
    frame_len = int(0.030 * CALMSEP_SR)
