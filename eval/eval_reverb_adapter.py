...
Usage (Lightning AI):
  cd /teamspace/studios/this_studio/calmsep
...
Usage (local Mac):
  ...
  .venv/bin/python eval/eval_reverb_adapter.py \
...
def _add_paths() -> None:
    """Add SR-CorrNet and calmsep to sys.path regardless of CWD."""
    repo = Path(__file__).resolve().parent.parent
    candidates = [
        repo,
        repo.parent / "SR_CorrNet_SS",
        Path("/teamspace/studios/this_studio/SR_CorrNet_SS"),
        Path("/teamspace/studios/this_studio/calmsep"),
    ]
    for p in candidates:
        s = str(p)
...
from data.calmsep_mixer import CalmSepMixture, MixtureRecipe  # type: ignore
from data.degradations import apply_reverb  # type: ignore
from data.mixer_stub import MixtureSample  # type: ignore
from data.rir_bank import RirBank  # type: ignore
from models.lora import LoRALayer, LoRALibrary  # type: ignore
from train.stage1_single import _forward_with_grad, _get_inner_module  # type: ignore
...
def diag_sisnr(
    ...
    for cond_name, t60 in conditions:
        ...
        if t60 is None:
            ...
            calmix = CalmSepMixture(sample=sample, recipe=recipe)
...
def diag_target(
    ...
    calmix = CalmSepMixture(sample=sample, recipe=MixtureRecipe(n_speakers=n_spks))
...
def main() -> None:
    ...
    print("=" * 60)
    print("  CALM-Sep Reverb Adapter Evaluation & Diagnosis")
    print("=" * 60)
    ...
    rev = apply_reverb(
        CalmSepMixture(sample=sample, recipe=MixtureRecipe(n_speakers=n_spks)),
        rir_bank,
        rng,
        t60_s=0.4,
    )
