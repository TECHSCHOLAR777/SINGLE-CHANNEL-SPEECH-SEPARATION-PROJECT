...
def test_download_skips_existing(tmp_path: Path) -> None:
    ...
    with patch("data.prepare_wham.urlretrieve") as mock_dl:
...
def test_download_calls_urlretrieve_when_missing(tmp_path: Path) -> None:
    ...
    with (
        patch("data.prepare_wham.urlretrieve", side_effect=fake_urlretrieve) as mock_dl,
        patch("data.prepare_wham.zipfile.ZipFile", side_effect=fake_zipfile),
    ):
        result = download_wham_noise(out)
...
def test_download_skips_download_when_archive_present(tmp_path: Path) -> None:
    ...
    with (
        patch("data.prepare_wham.urlretrieve") as mock_dl,
        patch("data.prepare_wham.zipfile.ZipFile", side_effect=fake_zipfile),
    ):
        result = download_wham_noise(out)
...
def test_download_raises_if_extraction_wrong_layout(tmp_path: Path) -> None:
    ...
    with (
        patch("data.prepare_wham.urlretrieve", side_effect=fake_urlretrieve),
        patch("data.prepare_wham.zipfile.ZipFile", side_effect=fake_zipfile),
    ):
