"""Tests for the CALM-Sep dual-branch preprocessor (Dev C)."""
...
class TestCalmSepPreprocess:
    def test_output_types(self):
        wav = _sine()
        result = calmsep_preprocess(wav, 16000)
        assert isinstance(result, CalmSepPreprocessedAudio)
    ...
    def test_8k_sample_rate_locked(self):
        wav = _sine()
        result = calmsep_preprocess(wav, 16000)
        assert result.sample_rate_8k == CALMSEP_SAMPLE_RATE
        # Length should correspond to 8 kHz.
        expected_len = int(2.0 * CALMSEP_SAMPLE_RATE)
    ...
    def test_8k_stft_bins(self):
        ...
        result = calmsep_preprocess(wav, 16000)
        # n_fft=128 → 65 bins
        assert result.stft_8k.shape[0] == CALMSEP_STFT_BINS
    ...
    def test_input_at_native_8k(self):
        wav = _sine(sr=8000)
        result = calmsep_preprocess(wav, 8000)
