"""
Unit tests for data/calmsep_mixer.py (Dev A, P0-A1).
...
from data.calmsep_mixer import (
    CALMSEP_SAMPLE_RATE,
    CalmSepMixer,
    CalmSepMixture,
    MixtureRecipe,
...
def test_codec_class_indices():
    from data.calmsep_mixer import _CODEC_CLASS_INDEX
...
def test_rejects_n_outside_2_to_5(tmp_path):
    ...
    with pytest.raises(ValueError, match="allowed_n"):
        CalmSepMixer(files, allowed_n=[1, 2])


def test_rejects_6_speakers(tmp_path):
    files = _build_pool(tmp_path, n_speakers=6)
    with pytest.raises(ValueError, match="allowed_n"):
        CalmSepMixer(files, allowed_n=[6])
...
def test_rejects_insufficient_pool(tmp_path):
    ...
    with pytest.raises(ValueError, match="training pool"):
        CalmSepMixer(files, allowed_n=[2])
...
def test_assert_speaker_isolation_passes(tmp_path):
    files = _build_pool(tmp_path, n_speakers=6)
    held = {"100", "101"}
    mixer = CalmSepMixer(files, held_out_speaker_ids=held)
...
def test_mix_returns_calmsep_mixture(tmp_path):
    files = _build_pool(tmp_path, n_speakers=6)
    mixer = CalmSepMixer(files, allowed_n=[2], rng=np.random.default_rng(0))
    m = mixer.mix()
    assert isinstance(m, CalmSepMixture)


def test_mix_recipe_n_speakers(tmp_path):
    files = _build_pool(tmp_path, n_speakers=6)
    for n in [2, 3, 4, 5]:
        mixer = CalmSepMixer(files, allowed_n=[n], rng=np.random.default_rng(n))
        m = mixer.mix(n=n)
...
def test_mix_references_shape(tmp_path):
    files = _build_pool(tmp_path, n_speakers=6)
    mixer = CalmSepMixer(files, allowed_n=[3], rng=np.random.default_rng(1))
    m = mixer.mix(n=3)
...
def test_mix_mixture_equals_sum_of_references(tmp_path):
    files = _build_pool(tmp_path, n_speakers=6)
    mixer = CalmSepMixer(files, allowed_n=[2], rng=np.random.default_rng(2))
    m = mixer.mix(n=2)
...
def test_mix_sample_rate_locked(tmp_path):
    files = _build_pool(tmp_path, n_speakers=6)
    mixer = CalmSepMixer(files, rng=np.random.default_rng(3))
    m = mixer.mix()
    assert m.sample.sample_rate == CALMSEP_SAMPLE_RATE
    assert m.recipe.sample_rate == CALMSEP_SAMPLE_RATE
...
def test_mix_rejects_wrong_sample_rate(tmp_path):
    ...
    with pytest.raises(ValueError, match="sample rate mismatch"):
        mixer = CalmSepMixer([bad, good], allowed_n=[2])
        mixer.mix(n=2)


def test_unique_utterance_ids(tmp_path):
    files = _build_pool(tmp_path, n_speakers=6)
    mixer = CalmSepMixer(files, rng=np.random.default_rng(7))
...
def test_heldout_split(tmp_path):
    ...
    held = {"100", "101"}
    mixer = CalmSepMixer(files, held_out_speaker_ids=held, allowed_n=[2])
